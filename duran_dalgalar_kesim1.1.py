...
y = np.array([2.10, 1.02, 0.72])
#y = np.array([0.1176, 0.05714, 0.04082])
x = np.array([1/8.500, 1/17.500, 1/24.500])
...
plt.text(0.045, 1.6, 'y = ' +  '{:.2f}'.format(a) + 'x' + ' + {:.2f}\n'.format(b),c = 'black', size=13)
#eğimi grafiğe bastırma
plt.text(0.045, 1.6, 'eğim =  {:.2f}'.format(a) + " m / s ",c = 'black', size=13)


plt.title("Grafik - 1 | F = 0,5 N ile Lambda λ (m) - 1/f (s)  Grafiği")
